...
export function timedPlayOptions(hand, leadSuit) {
  const matchingColor = leadSuit ? hand.filter((card) => card.suit === leadSuit) : [];
  if (matchingColor.length) return matchingColor;
  const submarines = hand.filter((card) => card.suit === 'sub');
  return submarines.length ? submarines : hand;
}
export function timedPlayCard(hand, leadSuit, random = Math.random) { const options = timedPlayOptions(hand, leadSuit); return options.length ? options[Math.floor(random() * options.length)] : null; }
...
export function taskStatus(task, state, ownerId) {
  ...
  if (task.type === 'atLeastTricks') return tricks.length >= task.amount ? 'complete' : (state.finished ? 'failed' : 'active');
  ...
  if (task.type === 'parityTrick') { const targetParity = task.parity === 'odd' ? 1 : 0; const complete = tricks.some((trick) => trick.cards.every((card) => card.suit !== 'sub' && card.rank % 2 === targetParity)); if (complete) return 'complete'; const remaining = Object.values(state.hands ?? {}).flat(); return state.finished || !remaining.some((card) => card.suit !== 'sub' && card.rank % 2 === targetParity) ? 'failed' : 'active'; }
...
export function taskSetStatus(tasks, state) {
  const statuses = tasks.map((task) => task.ownerId ? taskStatus(task, state, task.ownerId) : 'unassigned');
  const failedIndex = statuses.findIndex((status) => status === 'failed');
  return {
    statuses,
    failedTask: failedIndex < 0 ? null : tasks[failedIndex],
    allComplete: tasks.length > 0 && statuses.every((status) => status === 'complete')
  };
}
