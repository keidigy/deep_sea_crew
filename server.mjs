...
import { canPlayCard, createDeck, displayCard, drawTasks, shuffle, taskStatus, trickWinner } from './lib/game.mjs';
...
function publicRoom(room, viewerId) {
  ...
  return {
    code: room.code, version: room.version, status: room.status, hostId: room.hostId,
    players: room.players.map((member, seat) => ({ ...member, seat, handCount: game?.hands[member.id]?.length ?? 0 })),
    voice: { enabled: room.status === 'lobby', reason: room.status === 'lobby' ? '로비에서만 음성 연결 가능' : '기본 규칙: 플레이 중 대화 금지' },
    game: game && {
      captainId: game.captainId, leaderId: game.leaderId, turnId: game.turnId, missionDifficulty: game.missionDifficulty,
...
function startGame(room) {
  ...
  const missionDifficulty = 3;
  room.game = {
    hands, reserveCard, captainId: captain.id, leaderId: captain.id, turnId: captain.id, selectionTurnId: captain.id,
    missionDifficulty, tasks: drawTasks(count, missionDifficulty), currentTrick: [], trickHistory: [], completedTricks: 0,
    won: Object.fromEntries(room.players.map((member) => [member.id, []])), streaks: Object.fromEntries(room.players.map((member) => [member.id, 0])), result: null, finished: false
  };
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/create') {
      ...
      const host = player(id, data.name, true); rooms.set(code, { code, players: [host], hostId: id, status: 'lobby', game: null, version: 1 });
